...
def is_mail(text):
    ...
    for email in list_of_emails:  # go over the "emails" elements of the lst
        if email.count('@') == 1 and email[0] != '@':  # email valid check
            if email.count('.') == 1:  # email valid check
                return True
            else:
                return False
        else:
...
def main():
    ...
    lst_of_keys = read_file(file_of_keys).split(',')  # create a list of all the keys in the file
    print(lst_of_keys)

    for my_key in lst_of_keys:
        text_decrypted = decrypt_text(read_file(path_of_file_to_read), my_key)  # call & print the func that decrypt the file in read_file
        print(extract_mails_from_text(text_decrypted), "\n")  # print the emails we need (task 3) if it is a text of emails
