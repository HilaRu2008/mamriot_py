...
def decrypt_group(group_text, key):
    ...
    for letter in group_text:  # go over every letter in the group to give it different offset
        ...
        deciphered_letter_index = (encrypted_letter_index - int(offset)) % len(
            ABC)  # calculate the index of the deciphered letter (the letter we want to find)
        key_for_index += 1
...
def write_to_file(file_path, list_to_write):
    ...
    with open(file_path, "w") as file_of_mails:  # open the empty file of miles
...
def extract_mails_from_text(text):
    ...
    if is_mail(text):  # if this is the file with the emails
        ...
        for mail in lst_of_mails:  # go over every mail in the list
            if ("hotmail.com" in mail) or ("yahoo.com" in mail) or (
                    "gmail.com" in mail):  # if the email is the with the address we need
...
def is_mail(text):
    ...
    for email in list_of_emails:  # go over the "emails" elements of the lst
        if email.count('@') == 1 and email[0] != '@':  # email valid check
            if (email.count('.') == 1) and (email.index('.') > email.index('@')) and (
                    email.index('.') != email.index('@') + 1) and (email[-1] != '.'):  # email valid check
                return True
            else:
                return False
        else:
...
def main():
    ...
    while path_of_file_to_read != '999':
        ...
        for my_key in lst_of_keys:  # try every key to decrypt the file
            ...
            if extract_mails_from_text(text_decrypted) is not None:  # if the decrypted text contains emails
                lst_mails_we_need += extract_mails_from_text(text_decrypted)  # add to the list of emails we need the emails of a new file
    ...
    # add to a mails  file all the mails collected from all files to the list of mails + '\n'
    write_to_file(path_for_mails_needed, lst_mails_we_need)

    # use the read func to print all mails we had insert to the file of mails
    mails = read_file(path_for_mails_needed)
    print(mails)

    # calculate & print the amount of mails
    print( "the amount of emails overall: ", len(lst_mails_we_need)) # print the length of the list that collected all mails
